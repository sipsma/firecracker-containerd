	...
	*socketPath = filepath.Join(rootPath, "api.socket")
	...
	return firecracker.Handler{
		...
		Fn: func(ctx context.Context, m *firecracker.Machine) error {
			...
			newFirecrackerBinPath := filepath.Join(rootPath, filepath.Base(firecrackerBinaryPath))
			if err := copyFile(
				firecrackerBinaryPath,
				newFirecrackerBinPath,
				0500,
			); err != nil {
				return errors.Wrapf(err, "could not copy firecracker binary from path %v", firecrackerBinaryPath)
			}
